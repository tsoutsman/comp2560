...
use bindings::{
    ...
    policydb_set_target_platform, role_datum, type_datum, user_datum, AVRULE_ALLOWED,
    AVRULE_NEVERALLOW, DENY_UNKNOWN, POLICY_BASE, SEPOL_TARGET_SELINUX, SYM_CLASSES, SYM_ROLES,
    SYM_TYPES, SYM_USERS,
};
use model::{Class, Permission, Role, Rule, RuleType, Type, TypeSet, User};

pub fn new_policy() -> policydb {
...
pub fn classes(policy: &policydb) -> Vec<Class> {
...
pub fn types(policy: &policydb) -> Vec<Type> {
...
pub fn roles(policy: &policydb) -> Vec<Role> {
...
pub fn users(policy: &policydb) -> Vec<User> {
...
pub fn rules(policy: &policydb) -> Vec<Rule> {
